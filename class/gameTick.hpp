...
void gameTick() {
    while (Globals:: shouldTick) {
        auto start = std::chrono::high_resolution_clock::now();
        //do stuff, sleep for example.
        ...
        if(Globals::tickCount % 64 == 0) {
            std::cout << "Last tick completed in: " << (Globals::tickDelta / 1000) << "ms, " << "sleeping " << ((Globals::minimumTickDelta - Globals::tickDelta)  / 1000) << "ms." << std::endl;
